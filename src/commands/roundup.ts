...
async function roundup(interaction: discord.ChatInputCommandInteraction) {
    ...
    let ephemeral_var: boolean = true;
    ...
    let paginatedLists: Array<Array<String>> = [];
    let buffer: number = 0;
    let toPush: Array<String> = []
    let count: number = 0;
    let realCount: number = 0;
    ...
    let embeds: Array<discord.EmbedBuilder> = [];
    ...
    let defaultPage = 0;
    let defaultEmbed = embeds[defaultPage];
    ...
    let collector = mrwor.createMessageComponentCollector({ componentType: api.ComponentType.Button });
    ...
    collector.on('collect', async (buttonInteraction) => {
        ...
        let stringedNumber: string = footersplit.replace(/[^0-9]+/, "");
